...
class Users extends Controller
{
    ...
    public function formExtendFields($form)
    {
    	...
    	if(Input::segment(5) == "create"){
    	   ...
    	   $form->getField('timezone')->value = Config::get("app.timezone");
           $form->getField('country')->value = "PL";
           $form->getField('firm_country')->value = "PL";
           //$form->getField('partial2')->hidden = true;

        }

    	if(Input::get("type") == "addmodal") {

    		//$form->getField('avatar')->hidden = true;
    		//$form->getField('partial2')->hidden = true;
        ...
        if(!Util::mode("3")){
            $form->removeField('tax');
            $form->removeField('tax_type');
            $form->removeField('tax_office');
            $form->removeField('tax_amount');
            $form->removeField('tax_form');
            $form->removeField('tax_lump');
        }

    }

    /**
     * Display username column if settings permit
     */

    public function listExtendColumns($list)
    {

	if(isset($list->getColumns()['project'])) $list->getColumn('project')->label = e(trans($list->getColumn('project')->label))."&nbsp;";
     ...
     */
    public function update_onUnban($recordId = null)
    {
        $model = $this->formFindModelObject($recordId);
    ...
    public function update_onDelete($recordId = null)
    {
        ...
        Flash::success(Lang::get('backend::lang.form.delete_success',["name" => e(trans('rainlab.user::lang.user.label'))]));
     ...
     * 
     */
    public function update_onCreate($recordId = null)
    {
        $model = $this->formFindModelObject($recordId);

        return Redirect::to('/'.config('cms.backendUri').'/rainlab/user/users/update/'.$recordId);
    }
    ...
    public function onExport()
    {
        ...
        $url = config('cms.backendUri')."/eprog/manager/export/userxml";
