...
class Account extends ComponentBase
{
    ...
    public function onRegister()
    {
        ...
        try {
            ...
            $rules = [
     		...
     		'password_confirmation' => 'required_with:password|between:4,12',
	        'surname'    => 'required',
                'name'    => 'required'
            ];
	    ...
	    $this->captchaVerify();
            ...
            /*
             * Activation is by the admin, send the email  to admin
             */
            if ($adminActivation) {

                Flash::success(Lang::get('rainlab.user::lang.account.activation_admin'));
	...
	//throw new ApplicationException(Input::all());
...
*/
    ...
    public function onSendActivationEmail()
    {
        try {
            ...
            //Flash::success(Lang::get('rainlab.user::lang.account.activation_email_sent'));
          throw new ApplicationException(Lang::get('rainlab.user::lang.account.activation_email_sent'));
            $this->sendActivationEmail($user);
            ...
            //if (Request::ajax()) throw $ex;
            //else Flash::error($ex->getMessage());
            throw new ApplicationException(Lang::get('rainlab.user::lang.account.no_email'));
...
*/
    ...
    /**
     * Sends the register email to admin
     * @param  User $user
     * @return void
     */
    protected function sendRegisterEmail($user)
    {

	$receiver = MailSetting::get('sender_email');   	       
        ...
        Mail::send('rainlab.user::mail.new_user', $data, function($message) use ($user,$receiver) {
    ...
    protected function captchaVerify()
    {
            ...
            $captchalink = "https://www.google.com/recaptcha/api/siteverify?secret=".Config::get("app.captcha.secret")."&response=".Request::input('g-recaptcha-response');
            $response = file_get_contents($captchalink, false, stream_context_create(array('ssl' => array('verify_peer' => false, 'verify_peer_name' => false))));
            
            $response = json_decode($response, true);
            if(!isset($response['error-codes']) && $response['success'])
	    return true;
	    else
	    throw new ApplicationException(Lang::get('rainlab.user::lang.auth.captcha'));
