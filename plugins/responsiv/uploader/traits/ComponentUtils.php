...
trait ComponentUtils
{
    ...
    protected function checkUploadAction()
    {
	    ...
	    $this->setValidationRules();
 		...
 		['max_files' => e(trans('responsiv.uploader::lang.max_files'))]
            ...
            $file->is_public =  isset($this->model->{$relationType}[$this->attribute]['public']) ? $this->model->{$relationType}[$this->attribute]['public'] : true;
            $file->save();
    ...
    protected function setValidationRules()
    {
        ...
        if (is_null($this->rules) && method_exists($this->model, 'fileUploadRules'))
        {
            ...
            if (is_array($modelRules) && array_key_exists($this->attribute, $modelRules))
            {
                $this->rules = $modelRules[$this->attribute];
            }
        }
