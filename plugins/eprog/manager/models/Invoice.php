...
class Invoice extends Model
{
    ...
    public static $rule = [
    	...
    	'buyer_name'    => 'required',
    	'buyer_nip'     => 'required_unless:buyer_type,3|nip:Invoice.buyer_type,0|vateu:Invoice.buyer_type,1|ksef:Invoice.buyer_type,2',
    	...
    	'_addbuyer_nip'     => 'required_unless:_addbuyer_type,3|nip:Invoice._addbuyer_type,0|vateu:Invoice._addbuyer_type,1|ksef:Invoice._addbuyer_type,2',
    ...
    public function beforeSave() {
        ...
        if(Session::has("license.subscription") && Session::get("license.subscription") > 1 && Session::has("license.mode") && is_array(Session::get("license.mode")) && in_array("2a",Session::get("license.mode"))) {

            $month = date("n",strtotime(Input::get("Invoice.create_at")));
            $year = date("Y",strtotime(Input::get("Invoice.create_at")));
            $count = Invoice::whereMonth('create_at', $month)->whereYear('create_at', $year)->count();
            
            $max = config("app.invoice_max") ?? 10;
            if($count > $max)
            throw new ValidationException(['my_field'=>trans("eprog.manager::lang.ksef.invoice_max",["max" => $max])]);
