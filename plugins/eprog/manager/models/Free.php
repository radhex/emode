...
use Eprog\Manager\Controllers\Invoice as InvoiceController;
...
class Free extends Model
{
    ...
    public static $rule = [
        ...
        'buyer_name'    => 'required',
        'buyer_nip'     => 'required_unless:buyer_type,3|nip:Free.buyer_type,0|vateu:Free.buyer_type,1|ksef:Free.buyer_type,2',
        ...
        '_addbuyer_nip'     => 'required_unless:_addbuyer_type,3|nip:Free._addbuyer_type,0|vateu:Free._addbuyer_type,1|ksef:Free._addbuyer_type,2',
        ...
        '_pay_termin_ilosc' => 'integer|gt:0',
        '_pay_termin_jednostka' => 'required_with:_pay_termin_ilosc',
        '_pay_termin_zdarzeniepoczatkowe' => 'required_with:_pay_termin_ilosc',
        '_pay_other_desc' => 'required_if:_pay_type,7|max:256',
        '_pay_link' => 'paylink',
        '_pay_id' => 'ipksef',
        '_charge_reason' => 'required_with:_charge_amount',
        '_deduct_reason' => 'required_with:_deduct_amount',
    ...
    public $customMessages = [
        ...
        '_pay_termin_ilosc.gt' => 'eprog.manager::lang.greater_zero',
     ...
     */
    public $table = 'eprog_manager_invoice';

    protected $fillable = ["user_id","admin_id","nr","type","place","seller_name","seller_nip","seller_country","seller_adres1","seller_adres2","seller_email","seller_phone","buyer_name","buyer_type","buyer_nip","buyer_country","buyer_adres1","buyer_adres2","buyer_email","buyer_phone","brutto","netto","vat","currency","exchange","desc","ord","disp","make_at","create_at","ksef_at","xml","upo","ksefNumber","referenceNumber","invoiceReferenceNumber","created_at","updated_at"];


    public $hasMany = [
            'value' => ['Eprog\Manager\Models\Invoicevalue', 'delete' => true]
    ...
    public static $model = ["id" => "int(10)", "status_id" => "int(10)", "user_id" => "varchar(255)","admin_id" => "varchar(255)","nr" => "varchar(255)", "type" => "varchar(255)","place" => "varchar(255)","seller_name" => "varchar(255)", "seller_nip" => "varchar(255)", "seller_country" => "varchar(255)", "seller_adres1" => "varchar(255)","seller_adres2" => "varchar(255)","seller_email" => "varchar(255)",   "seller_phone" => "varchar(255)",   "buyer_name" => "varchar(255)", "buyer_type" => "varchar(255)", "buyer_nip" => "varchar(255)",  "buyer_country" => "varchar(255)",  "buyer_adres1" => "varchar(255)",   "buyer_adres2" => "varchar(255)",   "buyer_email" => "varchar(255)",    "buyer_phone" => "varchar(255)",    "brutto" => "decimal(10,2)", "netto" => "decimal(10,2)",      "vat" => "decimal(10,2)",    "currency" => "varchar(255)",   "exchange" => "decimal(10,4)",  "desc" => "mediumtext", "ord" => "int(11)", "disp" => "tinyint(1)",  "make_at" => "datetime",    "create_at" => "datetime", "ksef_at" => "datetime", "xml" => "longtext", "upo" => "text", "ksefNumber" => "varchar(255)", "referenceNumber" => "varchar(255)","invoiceReferenceNumber" => "varchar(255)", "created_at" => "timestamp", "updated_at" => "timestamp"];
    ...
    public function beforeSave() {
        ...
        if(Input::get("action") == "send"){
            ...
            if(isset($result['ksefNumber'])){    
                ...
                throw new ValidationException(['ksefNumber'=> $result['ksefNumber']]);
        
            }           
            else
                throw new ValidationException(['send_error'=>e(trans('eprog.manager::lang.ksef.send_error'))]);
    ...
    {
        $statues = InvoiceController::status();
    ...
    public function getStatusAttribute()
    {
        ...
        return  $this->status_id > 0 && isset(InvoiceController::status()[$this->status_id])  ? InvoiceController::status()[$this->status_id] : "";
    ...
    public function getTypeOptions()
    {
        ...
        if(Input::segment(5) == "create") {
                ...
                if(Input::has("correct_id")){
                    ...
                    if($invoice && strlen($invoice->ksefNumber) > 0) $kor = 1;                
                }

                if($kor && isset($invoice->type) && ($invoice->type == 0 || $invoice->type == 4))
                    return [1 => $lista[1]];
                else if($kor && isset($invoice->type) && $invoice->type == 2)
                    return [5 => $lista[5]];
                else if($kor && isset($invoice->type) && $invoice->type == 3)
                    return [6 => $lista[6]];
    ...
    public function getPayTerminJednostkaOptions(){

        return Util::getTermin();    
    ...
    public function updateValue()
    {
        ...
        Invoicevalue::where("invoice_id","=",$this->id)->delete();
        for($i = 0; $i < $length - 1;$i++){
            ...
            Invoicevalue::create(["invoice_id" => $this->id, "product" => Input::get('Values.edit_product.'.$i), "pkwiu" => Input::get('Values.edit_pkwiu.'.$i), "cn" => Input::get('Values.edit_cn.'.$i), "gtu" => Input::get('Values.edit_gtu.'.$i), "quantity" => Input::get('Values.edit_quantity.'.$i), "measure" => Input::get('Values.edit_measure.'.$i), "netto" =>  $netto, "vat" => Input::get('Values.edit_vat.'.$i), "brutto" => $brutto, "extra" =>  preg_replace(["/,/i","/\s/i"], [".", ""],  Input::get('Values.edit_extra.'.$i)), "exchange" =>  $exchange]);
...
/*
        ...
        $result = Ksef::invoiceSend($faktura);
...
*/
    ...
    public function onToggleActive()
    {
     ...
     file_put_contents("log","togle");
        $id = post('id');
        $checked = post('hecked') === 'true';

        $model = ModelInvoice::find($id);
        if ($model) {
            $model->switch = $checked;
            $model->save();
        }
    }
